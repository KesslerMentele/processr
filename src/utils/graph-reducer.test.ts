...
describe('graphReducer', () => {
  ...
  describe('SET_NODE_POSITION', () => {
    it('updates the node position', () => {
      ...
      const withNode = graphReducer(makeGraph(), { type: 'ADD_NODE', node });
      const result = graphReducer(withNode, { type: 'SET_NODE_POSITION', nodeId: node.id, position: { x: 100, y: 200 } });
      expect(result.nodes[node.id].position).toEqual({ x: 100, y: 200 });
    });

    it('is idempotent for a non-existent nodeId', () => {
      const graph = makeGraph();
      const result = graphReducer(graph, { type: 'SET_NODE_POSITION', nodeId: processrNodeId('ghost'), position: { x: 0, y: 0 } });
      expect(result).toBe(graph);
    });
    ...
    it('pushes a change to history.past', () => {
      const node = makeNode();
      const withNode = graphReducer(makeGraph(), { type: 'ADD_NODE', node });
      const result = graphReducer(withNode, { type: 'SET_NODE_POSITION', nodeId: node.id, position: { x: 10, y: 10 } });
      expect(result.history.past).toHaveLength(2);
    });
  });
  ...
  describe('UNDO', () => {
    ...
    it('reverses SET_NODE_POSITION', () => {
      ...
      const graph = applyActions([
        { type: 'ADD_NODE', node },
        { type: 'SET_NODE_POSITION', nodeId: node.id, position: { x: 100, y: 100 } },
      ]);
      const result = graphReducer(graph, { type: 'UNDO' });
